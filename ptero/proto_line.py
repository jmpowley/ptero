...
from PyQt6.QtWidgets import QApplication, QMainWindow, QWidget, QVBoxLayout, QLabel, QComboBox, QHBoxLayout, QPushButton, QMessageBox, QFileDialog, QListWidget, QListWidgetItem, QSpinBox
...
from PTERO.proto.data_io.sorting import sort_key
from PTERO.proto.data_io.extract_values import extract_quantity, extract_line_ratio

# Get the base path to Allen08
base_path = "/Users/Jonah/MISCADA/Project/code/PTERO/3mdbs_data/"

# Custom quantities to plot
custom_quantities = ["S23"]
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        layout.addWidget(QLabel("Select Abundance:"))
        ...
        layout.addWidget(self.abun_combo)

        # Density dropdown and label
        layout.addWidget(QLabel("Select Density:"))
        self.dens_combo = QComboBox()
        self.dens_combo.addItem("-")  # Default option
        self.dens_combo.currentIndexChanged.connect(self.update_mag_fields)
        layout.addWidget(self.dens_combo)
        
        # Magnetic dropdown and label
        layout.addWidget(QLabel("Select Magnetic Field:"))
        self.mag_combo = QComboBox()
        self.mag_combo.addItem("-")  # Default option
        layout.addWidget(self.mag_combo)

        # Create a QSpinBox and set its minimum, maximum, and initial value.
        layout.addWidget(QLabel("Min. Velocity"))
        ...
        # Create a QSpinBox and set its minimum, maximum, and initial value.
        layout.addWidget(QLabel("Max. Velocity"))
        ...
        self.tb_button = QPushButton("Load Table", self)
        self.tb_button.clicked.connect(self.on_tb_button_clicked)  # Connect to on_tb_button_clicked function
        layout.addWidget(self.tb_button)
        ...
        self.update_dropdowns()  # Ensure initial values are populated
    ...
    def update_dropdowns(self):
        ...
        # Automatically update magnetic fields based on the first density
        self.update_mag_fields()

    def update_mag_fields(self):
        abun_path = os.path.join(base_path, "Allen08", self.abun_combo.currentText())
        ...
        selected_density = self.dens_combo.currentText()

        # Filter magnetic field values based on selected density
        files = os.listdir(abun_path)
        res = [d.replace(self.abun_combo.currentText()+"_", "").replace(".csv", "").split("_") for d in files]
        magnetics = sorted(set(res[i][1] for i in range(len(res)) if res[i][0] == selected_density), key=float)

        # Clear and populate magnetic field dropdown
        self.mag_combo.clear()
        self.mag_combo.addItems(magnetics)
    ...
    def load_table(self):
        ...
        self.filepath = os.path.join(base_path, "Allen08", abun, f"{abun}_{dens}_{mag}.csv")

        if os.path.exists(self.filepath):
            try:
                self.df = pd.read_csv(self.filepath)
                df = self.df
                self.shocks = np.array((df.columns.values[1:]), dtype=int)

                # Get the sorted emission lines from the new table
                ...
                QMessageBox.information(self, "Success", "Table loaded successfully.")
    ...
    def plot_diagnostic(self):
        ...
        if xqulr == "Quantity":
            try:
                self.x_lab, self.x_data = extract_quantity(df, xquan, yquan, vmin, vmax, "x")
            except Exception as e:
                QMessageBox.critical(self, "Error", f"Failed to calculate X " + xqulr + f": {e}")
        elif xqulr == "Line Ratio":
            try:
                self.x_lab, self.x_data = extract_line_ratio(df, xnum, xden, ynum, yden, vmin, vmax, "x")
            except Exception as e:
                QMessageBox.critical(self, "Error", f"Failed to calculate X " + xqulr + f": {e}")
        if yqulr == "Quantity":
            try:
                self.y_lab, self.y_data = extract_quantity(df, xquan, yquan, vmin, vmax, "y")
            except Exception as e:
                QMessageBox.critical(self, "Error", f"Failed to calculate Y " + yqulr + f": {e}")
        elif yqulr == "Line Ratio":
            try:
                self.y_lab, self.y_data = extract_line_ratio(df, xnum, xden, ynum, yden, vmin, vmax, "y")
            except Exception as e:
                QMessageBox.critical(self, "Error", f"Failed to calculate Y " + yqulr + f": {e}")
    ...
    def plot_data(self):
        ...
        ax.set_xlabel(x_lab)
        ax.set_ylabel(y_lab)
        ax.set_title(f"{y_lab} against {x_lab}\nAbundance = {abun}; Density = {dens}" + " cm$^{-3}$" + f"; Magnetic = {mag} $\mu G$")
        ax.set_xscale('log')
        ax.set_yscale('log')

    def on_tb_button_clicked(self):
        self.load_table()

    def on_upload_clicked(self):
        # TODO: Look at structure of data file being uploaded
    ...
    def on_plot_button_clicked(self):
        if self.table_loaded:
            ...
            QMessageBox.critical(self, "Error", f"Please load a table to produce a plot")
